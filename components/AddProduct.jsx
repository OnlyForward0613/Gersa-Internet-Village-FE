...
const AddProduct = ({ ...pageProps }) => {
  ...
  }, [token, setToken, refreshToken, baseURL, router]);
      ...
      <div className="h-full flex flex-col items-center">
        <div className="w-full sm:w-[500px]">
          <div className="flex flex-row justify-between">
            <div className="font-excon text-3xl">Add Product</div>
            ...
            <div className=" flex flex-row font-pally z-0 transition-all ease-in-out duration-500 ">
              ...
              >
                <p className="break-words w-[250px] cursor-pointer">
                  Add stock options below if this product is not customizable
                </p>
              </Tooltip>
            </div>
            <table
          ...
          <span
            className="mx-auto flex w-fit h-14 mt-4"
            onClick={() => {
              if (otherImages.length > 5) {
                ...
                if (
                  ...
                  price > 0 &&
                  ((customizable.length > 0 && data.length <= 0) ||
                    (customizable.length <= 0 && data.length > 0))
                ) {
                  setFillError(false);
                      ...
                      .then((res) => {
                        res.json().then((data2) => {
                          if (data2.success == false) {
                            ...
                            fetch(
                              `${baseURL}/product/stock/${data2?.product?.id}`,
                              {
                                method: "POST",
                                headers: {
                                  Authorization: `Bearer ${merchantToken}`,
